...
def decode(string, key):
    metadata = json.loads(string.split("===")[0])
    string = string.split("===")[1]
    decoded_chars = []
    for i in range(len(string)):
        decoded_chars.append(chr((ord(string[i]) - ord(key[i % len(key)]) + 256) % 256))
    return "".join(decoded_chars), metadata
...
if __name__ == "__main__":
    ...
    if args.encrypt:
        ...
        if not args.filename[-5:] == ".enfc" :
            args.filename += ".enfc"
        dp = getpass("Decryption password: ")
        print(decrypt(args.filename, dp))
